...
// Show auth window on page reload  
window.onload = () => {
...
joinButton.addEventListener('click', () => {
    ...
    * 
    */
    // const socket = new WebSocket('wss://chatttt-room-demo.loca.lt');  // ${window.location.hostname}:3333
    const socket = new WebSocket(`${window.location.hostname}:3333`);
    ...
    sendButton.addEventListener('click', () => {
        ...
        // Send updated clipboard data to the server
        socket.send(JSON.stringify({ type: 'message', sender: usernameInput.value, content: message }));
...
// function DrawNeuralNetWorkBackGround() {
//     // Get 2D canva
